...
def rgbaf_color_to_int(rgb_color):
    red_f, green_f, blue_f, _ = rgb_color
    red = (round(red_f * 255) & 255)
    green = (round(green_f * 255) & 255)
    blue = (round(blue_f * 255) & 255)
...
def create_shaded_material(material, folder, file_name):
    ...
    if not os.path.exists(texture_file_path):
        raise Exception("Could not find the texture file '%s', was the .%s file moved to a folder without its textures?" % (file_name, FILE_EXTENSION))
...
# Hardcoded magic behavior numbers stored as the texture ID for the given material.
MAGIC_NO_TEXTURE_ID = -2
MAGIC_UNKNOWN_TEXTURE_ID = -1
def get_game_texture_id(material):
    if material is None:
    ...
    return MAGIC_UNKNOWN_TEXTURE_ID

def get_game_texture_name(material):
    if material is None:
        return None
    ...
    if re.fullmatch(r'tex([0-9]+)(\.([0-9]+))?', material.name, flags=re.IGNORECASE) is not None:
        return None # Prevent returning auto-generated names.
...
def load_map_file(operator, context, filepath):
    ...
    # Read map file into list of commands.
...
def save_map_file(operator, context, filepath):
    ...
    for polygon_index, polygon in enumerate(mesh.polygons):
        ...
        if is_textured:
            for i in range(polygon.loop_total):
                ...
                if uv[0] < -0.001 or uv[0] > 1.001 or uv[1] < -0.001 or uv[1] > 1.001:
                    operator.report({"WARNING"}, "Face %d has uvs[%d] out of the range which %s supports! (%f, %f)" % (polygon_index, i, GAME_DISPLAY_NAME, uv[0], uv[1]))
        ...
        if is_textured:
            ...
            writer.write(" %d" % (texture_flags or 0))
...
class SaveFroggerOperator(bpy.types.Operator, ExportHelper):
    ...
    bl_label = "Save %s Map (.%s)" % (GAME_DISPLAY_NAME, FILE_EXTENSION)
