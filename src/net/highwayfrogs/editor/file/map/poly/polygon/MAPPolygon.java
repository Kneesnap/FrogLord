...
public abstract class MAPPolygon extends MAPPrimitive implements TextureSource {
     ...
     */
    public static SVector getCenterOfPolygon(MAPPolygon poly) {
        if (poly == null)
            return null;
        ...
        for (int index : poly.getVertices()) {
            x += poly.getMapFile().getVertexes().get(index).getFloatX();
            y += poly.getMapFile().getVertexes().get(index).getFloatY();
            z += poly.getMapFile().getVertexes().get(index).getFloatZ();
        }
