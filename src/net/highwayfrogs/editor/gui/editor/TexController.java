 ...
 */
public class TexController extends EditorController<BeastWarsTexFile, BeastWarsInstance, SCGameConfig> {
    ...
    @Override
    public void loadFile(BeastWarsTexFile tex) {
        super.loadFile(tex);
    ...
    private void importImage(ActionEvent event) {
        ...
        try {
            ...
            th.printStackTrace();
            Utils.makePopUp("Failed to replace the image. (" + th.getMessage() + ")", AlertType.ERROR);
        }
    ...
    private void exportAllImages(ActionEvent event) {
        File directory = Utils.promptChooseDirectory("Select the directory to export images to.", true);
        if (directory == null)
            return; // Cancelled.
        ...
        for (File file : Utils.listFiles(selectedFolder)) {
            ...
            if (id >= 0 && id < getFile().getImages().size()) {
                try {
                    ...
                    th.printStackTrace();
                    Utils.makePopUp("Failed to replace the image with " + file.getName() + ". (" + th.getMessage() + ")", AlertType.ERROR);
                }
                importedFiles++;
            }
