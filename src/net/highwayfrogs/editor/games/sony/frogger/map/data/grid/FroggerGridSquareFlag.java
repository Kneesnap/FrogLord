...
public enum FroggerGridSquareFlag {
    ...
    SAFE(Constants.BIT_FLAG_1, FroggerGridSquareFlagTarget.BOTH, Color.BLUE, "Standard land. (Generally marks a tile as being possible to stand on)"), // 0x02
    DEADLY(Constants.BIT_FLAG_2, FroggerGridSquareFlagTarget.BOTH, Color.RED, "Kills the player using the generic flop death."), // 0x04
    ...
    POP_DEATH(Constants.BIT_FLAG_12, FroggerGridSquareFlagTarget.BOTH, Color.RED, "Kills the player with the pop animation. (Cacti)"), // 0x1000 This does work if used on a form, but forms use their own way of specifying deaths so this goes unused for forms.
    ...
    // These are only enabled if USABLE is set.
    @Getter private static final List<FroggerGridSquareFlag> usableReactionFlags = Arrays.asList(CLIFF_DEATH,
            SAFE, DEADLY, POP_DEATH, WATER, SLIPPY, SIMPLE_SLIPPY, FREEFORM_SLIPPY, BOUNCY);
