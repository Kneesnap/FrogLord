...
public class MoonWarriorLevelTableEntry implements ISCLevelTableEntry {
    ...
    private MWIResourceEntry vloEntry;
    ...
    public VloFile getVloFile() {
        if (this.vloEntry != null) {
            SCGameFile<?> gameFile = this.vloEntry.getGameFile();
        ...
        switch (this.mapFile.getFileDisplayName()) {
            case "TG.ENT":
            case "TG0.MAP":
                this.vloEntry = this.mapFile.getGameInstance().getResourceEntryByName("KG_TRAINING.VLO");
                break;
            case "WH2.ENT":
            case "WH20.MAP":
                this.vloEntry = this.mapFile.getGameInstance().getResourceEntryByName("WAREHOUSE2.VLO");
                break;
            default:
